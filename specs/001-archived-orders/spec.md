...
**Created**: 2026-04-12
**Status**: Draft
**Input**: User description: "quero que os pedidos que tenham mais de 7 dias sejam colocados em uma rota '/arquivados' e os cards fiquem la"

## User Scenarios & Testing *(mandatory)*

### User Story 1 - View Archived Orders (Priority: P1)

An operator navigates to the `/arquivados` route and sees all orders that were created more than 7 days ago, displayed as order cards in the same visual format used elsewhere in the system.
...
**Independent Test**: Can be fully tested by navigating to `/arquivados` after the system has orders older than 7 days, and verifying that those orders appear as cards on the page.

**Acceptance Scenarios**:

1. **Given** the operator is authenticated and there are orders older than 7 days, **When** they navigate to `/arquivados`, **Then** only orders with a creation date more than 7 days ago are displayed as order cards.
2. **Given** the operator is authenticated and there are no orders older than 7 days, **When** they navigate to `/arquivados`, **Then** an empty state message is displayed, informing the operator that there are no archived orders.
3. **Given** the operator is unauthenticated, **When** they navigate to `/arquivados`, **Then** they are redirected to the sign-in page.
...
2. **Given** all existing orders are older than 7 days, **When** the operator views the main orders page, **Then** the orders list is empty (or shows an appropriate empty state), and all orders appear on `/arquivados`.

---

### User Story 3 - Navigate to Archived Orders (Priority: P3)

An operator can navigate to the `/arquivados` route directly from the main navigation, without needing to manually type the URL.
...
**Independent Test**: Can be fully tested by checking that at least one navigational affordance (link, menu item, or button) in the application leads to `/arquivados`.

**Acceptance Scenarios**:

1. **Given** the operator is on any page in the application, **When** they look for archived orders in the navigation, **Then** a clearly labeled link or menu entry for archived orders is visible and navigates them to `/arquivados`.
...
- What if the operator's session expires while viewing `/arquivados`? The system must redirect them to sign-in.
...
- **FR-001**: The system MUST expose a new protected route at `/arquivados` that is only accessible to authenticated users.
- **FR-002**: The `/arquivados` route MUST display order cards for all orders whose creation date is strictly more than 7 days before the current date and time.
...
- **FR-004**: The order cards on `/arquivados` MUST present the same information and visual format as order cards used on other pages in the system.
- **FR-005**: The `/arquivados` page MUST display a clear empty-state message when there are no orders older than 7 days.
- **FR-006**: The `/arquivados` page MUST display a loading state while orders are being fetched.
- **FR-007**: The `/arquivados` page MUST display an error state with a retry affordance if the data request fails.
- **FR-008**: The system MUST preserve Brazilian-localized date, time, currency, and order status presentation on all cards shown in `/arquivados`.
...
- **SC-001**: 100% of orders older than 7 days are absent from the main orders view and present on `/arquivados` after the feature is deployed.
- **SC-002**: The `/arquivados` page loads and displays order cards within the same time as any other orders page in the application.
...
- **SC-005**: Unauthenticated access to `/arquivados` is redirected to sign-in in 100% of test attempts.
...
- The `/arquivados` route is in Portuguese to match the application's target locale (Brazil).
